...
import { LayoutDashboard, UserPlus, Layers } from "lucide-react";
...
const TeacherSidebar = () => {
  ...
  const menuItems = [
    ...
    { icon: UserPlus, label: "Request student", path: "/teacher/request-student" },
  ];
    ...
    >
      <div className={cn("border-b border-sidebar-border/30 transition-all duration-300", isCollapsed ? "p-4" : "p-6")}>
        <div className="flex items-center gap-2">
          ...
          />
          {!isCollapsed && <h1 className="text-xl font-bold text-sidebar-foreground whitespace-nowrap">LMS Teacher</h1>}
        </div>
        {!isCollapsed && <p className="text-sm text-sidebar-foreground/70 mt-1">Teacher portal</p>}
      </div>
      ...
      <div className="p-4 border-t border-sidebar-border/30">
        ...
        >
          ...
          {!isCollapsed && (
            <div>
              <p className="font-medium text-sm text-sidebar-foreground">Teacher</p>
              <p className="text-xs text-sidebar-foreground/60">Courses &amp; roster</p>
            </div>
          )}
        </div>
      </div>
